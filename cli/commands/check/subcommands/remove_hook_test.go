...
func TestRemoveCheckHookCommandRunEClosureWithFlags(t *testing.T) {
	...
	client.On("RemoveCheckHook", mock.AnythingOfType("*v2.CheckConfig"), "non-zero", "hook1").Return(nil)
...
func TestRemoveCheckHookCommandRunEClosureWithServerErr(t *testing.T) {
	...
	cli := test.NewMockCLI()
	client := cli.Client.(*client.MockClient)
	client.On("RemoveCheckHook", mock.AnythingOfType("*v2.CheckConfig"), "non-zero", "hook1").Return(errors.New("oh noes"))
	client.On("FetchCheck", "name").Return(types.FixtureCheckConfig("name"), nil)
